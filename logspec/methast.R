...
parallel.combine.methast <- function(prefix, seeds, yy, xxs, zzs, adm1, adm2, weights=1, iter=600, warmup=100, prior=noninformative.prior) {
    ...
    for (seed in 1:seeds) {
        ...
        this.likeli <- calc.likeli.nosigma(K, L, dmxxs, dmyy, zzs, adm1, methast.result$betas[methast.result$best.index,], matrix(methast.result$gammas[methast.result$best.index,], K, L), weights, prior)
        if (this.likeli > best.likeli) {
